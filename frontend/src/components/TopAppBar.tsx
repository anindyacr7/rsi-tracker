    ...
    <header className="fixed top-0 w-full z-50 bg-surface-container/80 dark:bg-surface-container/80 backdrop-blur-md border-b border-outline-variant/30 flex justify-between items-center px-margin-mobile md:px-margin-desktop h-16 transition-colors">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          title={pushStatus === 'granted' ? 'Notifications Enabled' : 'Enable Notifications'}
          className={clsx(
            "flex items-center justify-center p-2 rounded-full transition-colors active:scale-95",
            pushStatus === 'granted' ? "text-secondary bg-secondary/10" : "text-on-surface-variant hover:bg-surface-container-highest/50"
          )}
        >
          <span className="material-symbols-outlined" style={pushStatus === 'granted' ? { fontVariationSettings: "'FILL' 1" } : {}}>
            notifications
          </span>
        </button>
